...
export class RadioStationService {
  ...
  public async createArtistStation(artist: string, tracks: MusicLibraryEntry[]): Promise<RadioStation> {
    ...
    await radioStationService.updateStationFromTracks(tempStation, tracks, {
      album: 0,
    });
    return tempStation;
  }
  ...
  public async createAlbumStation(artist: string, album: string, tracks: MusicLibraryEntry[]): Promise<RadioStation> {
    // Create a temporary station name
    const stationName = `Station from ${artist} - ${album}`;

    // Create the temporary radio station with default criteria
    const tempStation = await radioStationService.createStation({
      name: stationName,
      description: `Temporary station based on "${album}" by "${artist}"`,
      criteria: [],
      isAutoGenerated: true,
      isTemporary: true
    });
